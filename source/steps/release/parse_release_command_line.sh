...
function parse_release_command_line
{
    ...
    declare -n option_releases_path="$4"
    declare -n option_shared_path="$5"
...
===============
    ...
    ${yellow}Arguments:${reset_foreground}
      ${green}stage-config-file${reset_foreground}       $option_config_file
        ${green}DEPLOY_MAX_RELEASES${reset_foreground}   $DEPLOY_MAX_RELEASES
      ...
      ${green}archive-file${reset_foreground}            $option_archive_file
...
function release_option_load_config_file
{
    declare -r file="$1"
...
function release_option_validate_archive_file
{
    declare -n file="$1"
...
function release_option_validate_paths
{
...
function release_option_validate_servers
{
...
function release_option_validate_shared_items
{
    if ! [[ -v DEPLOY_SHARED_ITEMS ]]
    then
...
function release_option_validate_release_number
{
    ! [[ -v DEPLOY_MAX_RELEASES ]] && DEPLOY_MAX_RELEASES="0"

    if ! [[ "$DEPLOY_MAX_RELEASES" =~ ^[0-9]+$ ]]
    then
        error "Invalid DEPLOY_MAX_RELEASES variable."
...
function release_option_validate_connect_option
{
